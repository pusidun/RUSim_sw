...
void SPIWrite(u16 addr, u8 val)
{
	...
	if((regVal&0xC0) == 0xC0)
	{
		...
		Write_Data(BaseAddr, 0x0024, regVal);
		//配置0x0028 , bit1 先写1 再写0
		Write_Data(BaseAddr, 0x0028, 0x01);delay_ad9362(2);
		Write_Data(BaseAddr, 0x0028, 0x00);
	}
	//read 0x0024 if bit7:6 is 0x11 , 表示SPI控制器是空闲状态，可以读写
	regVal = Read_Data(BaseAddr, 0x0024);
...
void WAIT_CALDONE(u16 index)
{
	...
	{
		//WAIT_CALDONE	RXFILTER,2000	// Wait for RX filter to tune, Max Cal Time: 5.585 us (Done when 0x016[7]==0)

	}
	else if(index == 5)
	{
		//WAIT_CALDONE	TXFILTER,2000	// Wait for TX filter to tune, Max Cal Time: 2.889 us (Done when 0x016[6]==0)
